...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        ...
        vol.Optional(CONF_CUSTOM_TEMP_ENTITY_ID): cv.entity_id,
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    custom_temp_entity_id = config.get(CONF_CUSTOM_TEMP_ENTITY_ID) # Read custom temperature sensor entity ID  # Read custom temperature sensor entity ID
    ...
    async_add_entities(
        [
            ProxyClimateThermostat(
                name,
                climate_entity_id,
                custom_temp_entity_id,   # Pass custom temperature sensor entity ID
                target_temp_high,
                target_temp_low,
                min_cycle_duration,
                cold_tolerance,
...
class ProxyClimateThermostat(ClimateEntity, RestoreEntity):
    ...
    def __init__(
            ...
            name,
            climate_entity_id,
            custom_temp_entity_id,  # Add custom temperature sensor entity ID
            target_temp_high,
            target_temp_low,
            min_cycle_duration,
            cold_tolerance,
        ...
        self.custom_temp_entity_id = custom_temp_entity_id  # Store custom temperature sensor entity ID
    ...
    async def async_added_to_hass(self):
        ...
        # Add listener for custom temperature sensor entity state, if provided
        if self.custom_temp_entity_id:
            self.async_on_remove(
                async_track_state_change(
                    self.hass, self.custom_temp_entity_id, self._async_custom_temp_state_changed
                )
            )
    ...
    @callback
    def _async_climate_state_changed(self, entity_id, old_state, new_state):
        """Handle changes in the climate entity state."""
        # Only update the current temperature if a custom temperature sensor is not provided
        if not self.custom_temp_entity_id:
            super()._async_climate_state_changed(entity_id, old_state, new_state)

    @callback
    def _async_custom_temp_state_changed(self, entity_id, old_state, new_state):
        """Handle changes in the custom temperature sensor entity state."""
        # Update the temperature only if the custom temperature sensor entity is provided
        if new_state is None or new_state.state in (STATE_UNAVAILABLE, STATE_UNKNOWN):
            _LOGGER.debug("New custom temperature entity state is not available")
            return

        if ATTR_TEMPERATURE in new_state.attributes and (temp := new_state.attributes[ATTR_TEMPERATURE]) is not None:
            self._async_update_temp(temp)  # Update the temperature from the custom sensor
        else:
            _LOGGER.debug("Current temperature is not present in custom temperature entity state")

        # Check the custom temperature sensor state for comfort conditions or mode overrides
        self._async_control_heating(force=True)
        self.async_write_ha_state()
